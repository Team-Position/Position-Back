...
// 공통 DOM 요소
const memberKeywordInput = document.getElementById("memberSearchInput");
const corporationKeywordInput = document.getElementById("corporationSearchInput");
const memberListContainer = document.querySelector(".UserTable_container");
const corporationListContainer = document.querySelector(".CorporationTable_container");
const memberPaginationContainer = document.querySelector(".pagination-list.member");
const corporationPaginationContainer = document.querySelector(".pagination-list.corporation");
const memberSortOptions = document.querySelectorAll("#user-section .sort-filter-option");

// 정렬 상태
let selectedMemberSort = "recent";
let selectedCorporationSort = "가입일 순";

// 공통 데이터 표시 함수
const renderListAndPagination = (listContainer, paginationContainer, data, renderList, fetchFunction) => {
    renderList(data.items);
    renderPagination(paginationContainer, data.pagination, fetchFunction);
};
...
<!--//             <div class="UserTable_cell">Action</div>-->
    ...
    members.forEach((member) => {
        html += `
            <div class="UserTable_row">
                <div class="UserTable_cell">${member.memberName || "N/A"}</div>
                <!-- 추가 데이터 -->
            </div>
        `;
    });
...
// 기업 회원 데이터 표시
const renderCorporationList = (corporations) => {
    let html = `
        <div class="CorporationTable_row CorporationTable_header">
            <!-- 헤더 내용 -->
        </div>
    ...
    corporations.forEach((corporation) => {
        html += `
            <div class="CorporationTable_row">
                <div class="CorporationTable_cell">${corporation.corporationName || "N/A"}</div>
                <!-- 추가 데이터 -->
            </div>
        `;
    });
    corporationListContainer.innerHTML = html;
};

// 페이징 처리
const renderPagination = (container, pagination, fetchFunction) => {
    let html = "";
    const { currentPage, totalPages, startPage, endPage } = pagination;

    if (currentPage > 1) {
        html += `<li><a href="#" onclick="${fetchFunction.name}(1)">처음</a></li>`;
        html += `<li><a href="#" onclick="${fetchFunction.name}(${currentPage - 1})">이전</a></li>`;
    }
    ...
    for (let i = startPage; i <= endPage; i++) {
        if (i === currentPage) {
            ...
            html += `<li><a href="#" onclick="${fetchFunction.name}(${i})">${i}</a></li>`;
        }
    }

    if (currentPage < totalPages) {
        html += `<li><a href="#" onclick="${fetchFunction.name}(${currentPage + 1})">다음</a></li>`;
        html += `<li><a href="#" onclick="${fetchFunction.name}(${totalPages})">끝</a></li>`;
    }
...
const fetchAndShowMembers = (page) => {
    ...
    memberService.fetchMembers(page, keyword, selectedMemberSort, (data) => {
        renderListAndPagination(memberListContainer, memberPaginationContainer, data, renderMemberList, fetchAndShowMembers);
    });
};

const fetchAndShowCorporations = (page) => {
    const keyword = corporationKeywordInput.value.trim();
    memberService.fetchCorporationMembers(page, keyword, (data) => {
        renderListAndPagination(corporationListContainer, corporationPaginationContainer, data, renderCorporationList, fetchAndShowCorporations);
    });
};
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    memberKeywordInput.addEventListener("input", () => fetchAndShowMembers(1));
    corporationKeywordInput.addEventListener("input", () => fetchAndShowCorporations(1));

    memberSortOptions.forEach((option) => {
        option.addEventListener("click", () => {
            selectedMemberSort = option.getAttribute("data-type");
            fetchAndShowMembers(1);
        });
    });
});
